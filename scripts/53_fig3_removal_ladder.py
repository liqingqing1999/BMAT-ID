...
fig = plt.figure(figsize=(8.4, 4.6), dpi=300)
gs  = fig.add_gridspec(1, 2, width_ratios=[1.45, 1.0], wspace=0.30,
                       left=0.105, right=0.975, top=0.865, bottom=0.26)
...
# the baseline annotation hugs the left end of its own dotted line.  The legend
# now lives under the figure (fig.legend, at the end), so the panel no longer needs
# in-axes head-room for it and a narrower canvas can be used.  This figure is
# placed at the 5.83 in text width, so the canvas width sets how far the lettering
# shrinks: 0.69x here, against 0.53x for the earlier 11 in canvas, which had left
# the smallest labels at ~4 pt.
ax.text(300, base_k + 80, "all genes: %d site-dependent" % base_k,
        color=C_BASE, fontsize=9.0, va="bottom")
...
for x, y in zip(remove, kept):
    ax.annotate("%d" % y, (x, y), textcoords="offset points", xytext=(0, -17),
                ha="center", fontsize=9.0, color=C_SITE, fontweight="bold")
...
for x, y in zip(rnd_x, rnd_y):
    ax.annotate("%d" % y, (x, y), textcoords="offset points", xytext=(0, 10),
                ha="center", fontsize=9.0, color=C_RAND, fontweight="bold")

ax.set_xlabel("number of genes removed from the model", fontsize=9.6)
ax.set_ylabel("genes with a site effect\n(FDR < 0.05, limma-voom)", fontsize=9.6)
ax.set_title("a   Attribution of the in vivo site signal", fontsize=10.6, loc="left",
             fontweight="bold", pad=8)
ax.set_xlim(-350, 7600); ax.set_ylim(-140, 3250)
ax.tick_params(labelsize=9.0)
ax.spines[["top", "right"]].set_visible(False)
# no in-axes legend: both series are labelled in the shared legend under the figure
...
ax2.set_xticks(xs); ax2.set_xticklabels(lbl, fontsize=9.0)
ax2.set_ylabel("site-dependent genes retained\n(% of the 2,712 baseline)", fontsize=9.6)
# two lines, not one: on this canvas the 41-character single-line title would run
# past the right edge of the figure (panel b is only ~2.9 in wide)
ax2.set_title("b   Same number of genes,\ndifferent genes", fontsize=10.6, loc="left",
              fontweight="bold", pad=8)
ax2.set_ylim(0, 108)
ax2.tick_params(labelsize=9.0)
ax2.spines[["top", "right"]].set_visible(False)
ax2.text(0.5, -0.20, "removing 2,975 / 7,070 genes", transform=ax2.transAxes,
         ha="center", fontsize=8.6, color="#555555")

# ---- shared legend for both panels, placed under the figure -----------------
h_, l_ = ax.get_legend_handles_labels()
fig.legend(h_, l_, loc="lower center", bbox_to_anchor=(0.5, 0.005), ncol=2,
           frameon=False, fontsize=8.8, handlelength=2.4, columnspacing=2.4)

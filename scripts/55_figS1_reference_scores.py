...
fig, axes = plt.subplots(1, 2, figsize=(9.0, 4.2), gridspec_kw=dict(width_ratios=[1.45, 1.0], wspace=0.26))
ax = axes[0]
...
ax.text(5.5, ax.get_ylim()[1] * 0.94, 'in vivo (n = 12)', ha='center', fontsize=9.0, color='#333')
ax.text(17.5, ax.get_ylim()[1] * 0.94, 'in vitro (n = 12)', ha='center', fontsize=9.0, color='#333')
ax.set_xticks(x); ax.set_xticklabels(lbl, rotation=90, fontsize=7.6)
ax.set_ylabel('Reference-anchored score\n(mean z x sqrt(n))', fontsize=8.4)
ax.legend(fontsize=7.8, frameon=False, ncol=4, loc='upper center', bbox_to_anchor=(0.5, -0.46))
ax.set_title('All four reference signatures collapse in vitro', fontsize=9.6, loc='left', fontweight='bold')
...
ax2.set_xlabel('in vivo score', fontsize=8.4); ax2.set_ylabel('in vitro score', fontsize=8.4)
ax2.set_title('Donor-matched in vivo vs in vitro', fontsize=9.6, loc='left', fontweight='bold')
ax2.legend(fontsize=7.8, frameon=False, loc='lower right')
for sp in ('top', 'right'):

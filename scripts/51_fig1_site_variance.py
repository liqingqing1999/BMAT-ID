...
for i in range(N):
    ax.text(i, 101.5, '%.1f' % data[i]['loc'], ha='center', va='bottom',
            fontsize=7.4, fontweight='bold', color='#2F4B7C')
ax.set_xticks(range(N)); ax.set_xticklabels(names, fontsize=7.2)
ax.set_ylabel('Variance component\n(mean % of gene variance)', fontsize=7.8)
ax.set_ylim(0, 112); ax.set_xlim(-0.6, N - 0.4); ax.set_yticks([0, 20, 40, 60, 80, 100])
fig.legend(handles=handles + [rb], fontsize=6.9, frameon=False, loc='upper center',
           bbox_to_anchor=(0.5, 0.988), ncol=5, columnspacing=0.9, handlelength=1.0,
           handletextpad=0.4)
ax.set_title('a  Variance composition', fontsize=8.8, loc='left', fontweight='bold')
...
ax.set_xlabel('Site-attributable variance per gene (%)', fontsize=7.8)
ax.set_ylabel('Cumulative fraction of genes', fontsize=7.8)
...
ax.text(97, 0.72, '%.1f%% of genes: site term = 0\n(%.1f%% with reference)\n'
        ...
        fontsize=7.0, ha='right', va='top', color='#777')
ax.legend(fontsize=6.9, frameon=False, loc='lower right')
ax.set_title('b  Site variance per gene', fontsize=8.8, loc='left', fontweight='bold')

...
for i in range(3):
    ax.text(i - w / 2, n_m1[i] * 1.35, '%d' % n_m1[i], ha='center', fontsize=6.8, color='#4A5B78')
    ax.text(i + w / 2, n_m3[i] * 1.35, '%d' % n_m3[i], ha='center', fontsize=6.8, color='#555')
...
ax.legend(fontsize=6.6, frameon=False, loc='upper right', borderaxespad=0.15)
...
for gn, g in groups_c.items():
    ...
    for (name, a, b), ly in zip(g['genes'], lab_y):
        ...
        axc.text(X0 + o - 0.075, ly, name, fontsize=6.2, color=g['color'], ha='right',
                 va='center', zorder=6, bbox=dict(facecolor='white', edgecolor='none',
                                                  pad=0.10, alpha=0.88))
...
for gn, ly in zip(order, lab_ys):
    ...
    axc.text(X1 + o + 0.06, ly, '%s\n%.2f \u2192 %.2f' % (gn, mm1, mm3), fontsize=6.6,
             color=col, va='center', fontweight='bold', zorder=9,
             bbox=dict(facecolor='white', edgecolor='none', pad=0.22, alpha=0.92))
axc.text(-0.88, 0.026, 'variance \u2192 0', fontsize=6.3, color='#888888', va='bottom')
